...
Implement Vaadin's `Converter` support in auto-generated forms. The `Field.converter(...)` API already exists but it is not currently applied when binding fields.
        ...
        .form(AutoGeneratedFormFactory.of(User.class)
                .fields(
                        Field.of(User::getSalary, User::setSalary, BigDecimal.class)
                                .label("Salary")
                                .converter(new StringToBigDecimalConverter("Invalid salary. Cannot convert to BigDecimal."))))
        ... other configs ...
        .build();
```
...
- Existing field customization APIs must continue working.
